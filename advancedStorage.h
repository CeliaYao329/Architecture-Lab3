...
typedef struct StorageStats_ {
  ...
  int fetch_num ;      // Fetch lower layer 对下层存储的访问次数
...
class Storage {
 public:
  ...
  virtual void HandleRequest(unsigned int addr, int bytes, int read,
                             char *content, int &hit, int &time, bool isFirst) = 0;
